		...
		"about": {
			...
			renderHtml: function (data, context) {
				...
				if (data.property("website").defined()) {
					...
					var websites = data.propertyValue("website").split(/[\s]+/g);
					for (var i = 0; i < websites.length; i++) {
						if (i > 0) {
							result += '<br>';
						}
						result += '<a href="' + websites[i] + '">' + websites[i] + '</a>';
					}
					result += '</div>';
				}
				return result;
...
Jsonary.render.register({
	...
	renderHtml: function (data, context) {
		// Render header
		var result = '<div class="facebook-user-link">';
		var linkHtml = "";
		var pictureLink = data.links("picture")[0];
		linkHtml += '<img class="facebook-user-link-picture" src="' + pictureLink.href + '">';
		linkHtml += '<span class="facebook-user-link-name">' + context.renderHtml(data.property("name")) + '</span>';
		result += context.actionHtml(linkHtml, "follow-link");
		result += '</div>';
		return result;
	},
	action: function (context, actionName, arg1) {
	...
	filter: function (data, schemas, uiState) {
		return schemas.containsUrl("user.json#/definitions/reference");
	}
});
