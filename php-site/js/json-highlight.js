(function () {
	...
	Highlighter.prototype = {
		...
		highlightJson: function (keywords, assumeKeyword) {
			...
			if (next == "{") {
				...
				this.highlightString(assumeKeyword);
		...
		highlightString: function (keyword) {
			this.html += '<span class="highlight-json-punctuation">&quot;</span>';
			this.html += keyword ? '<span class="highlight-json-keyword">' : '<span class="highlight-json-string">';
			next = this.next();
			while (next != '"' && next != '&quot') {
				if (next == "\\") {
					this.html += next;
	...
	function highlightElement(element, keywords) {
		...
		try {
			highlighter.highlightJson(keywords, true);
		} catch (e) {
			throw e;
		}
